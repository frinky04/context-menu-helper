...
mod windows {
    ...
    impl RegistryProvider for WindowsRegistryProvider {
        fn scan_entries(&self) -> Result<Vec<MenuEntry>> {
            ...
            for (hive_name, base_path) in SAFE_SCAN_PATHS {
                ...
                if *base_path == r"Software\Classes\SystemFileAssociations" {
                    scan_system_file_associations(hive_name, &base, &mut entries)?;
                    continue;
                }
    ...
    fn scan_system_file_associations(
        ...
        for ext in root.enum_keys().flatten() {
            ...
            let shell_key = match root.open_subkey(format!(r"{}\shell", ext)) {
                Ok(key) => key,
                Err(_) => continue,
            };
